...
def assemble(opcode: str, args: list[str]) -> int:
    ...
    # psuedo-instructions
        ...
        print(f"test.basm:{line_number}: unknown opcode {opcode!r}")
        exit(1)
...
def assemble_operation(op: str) -> int:
    if op == "or":
        ...
        print(f"test.basm:{line_number}: unknown operation {op!r}")
        exit(1)
...
def assemble_condition(cond: str) -> int:
    if cond == "eq":
        ...
        print(f"test.basm:{line_number}: unknown condition {cond!r}")
        exit(1)
...
def assemble_offset(offset: str) -> int:
    ...
    try:
        ...
        print(f"test.basm:{line_number}: expected offset, got {offset!r}")
        exit(1)

    if offset_int < -32 or offset_int > 31:
        print(f"test.basm:{line_number}: offset must be between -32 and 31")
        exit(1)
...
def assemble_address(addr: str) -> int:
    try:
        ...
        if addr not in labels:
            print(f"test.basm:{line_number}: unknown label or invalid integer address {addr!r}")
            exit(1)
        addr_int = labels[addr]

    if addr_int < 0 or addr_int >= 2**10:
        print(f"test.basm:{line_number}: address must be between 0 and 1023")
        exit(1)
...
def assemble_immediate(imm: str, port: bool = False) -> int:
    ...
    try:
        ...
        print(f"test.basm:{line_number}: expected {expected}, got {imm!r}")
        exit(1)

    if imm_int < -128 or imm_int > 255:
        print(f"test.basm:{line_number}: {expected} must be between -128 and 255")
        exit(1)
...
def assemble_reg(reg: str) -> int:
    if reg[0] != "r" or not reg[1:].isdigit():
        print(f"test.basm:{line_number}: expected register, got {reg!r}")
        exit(1)

    reg_num = int(reg[1:])

    if reg_num >= 8:
        print(f"test.basm:{line_number}: register number must be less than 8")
        exit(1)
...
for line in source.splitlines():
    ...
    while ":" in line:
        ...
        if label in labels:
            print(f"test.basm:{line_number}: label {label!r} already defined")
            exit(1)
...
for line in source.splitlines():
    ...
    if get_current_address() >= 2**10:
        print(f"test.basm:{line_number}: program too long")
        exit(1)
    ...
    if any([" " in arg for arg in args]):
        print(f"test.basm:{line_number}: missing comma between arguments")
        exit(1)
